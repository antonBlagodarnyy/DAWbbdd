...
--4- Emplee una subconsulta con el operador "exists" para devolver la lista de 
--socios que se inscribieron en un determinado deporte.
SELECT nombre
FROM socios
WHERE EXISTS (SELECT deporte FROM inscriptos WHERE INSCRIPTOS.numeroSocio = SOCIOS.numero AND deporte = 'futbol');

--5- Busque los socios que NO se han inscripto en un deporte determinado empleando "not exists".
SELECT nombre
FROM socios
WHERE  NOT EXISTS (SELECT deporte FROM inscriptos WHERE INSCRIPTOS.numeroSocio = SOCIOS.numero AND deporte = 'futbol');
...
WHERE NOT EXISTS (
    SELECT *
    FROM inscriptos
    WHERE inscriptos.numeroSocio = socios.numero
    AND inscriptos.cuotas > 0
);

--7- Obtenga el mismo resultado de la consulta anterior pero esta vez emplee una combinación.
SELECT s.*
FROM socios s
LEFT JOIN inscriptos i ON s.numero = i.numeroSocio AND i.cuotas > 0
WHERE i.numeroSocio IS NULL;
